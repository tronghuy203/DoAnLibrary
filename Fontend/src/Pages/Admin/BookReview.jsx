...
import { PencilIcon, TrashIcon, BookOpenIcon, CheckCircleIcon, ExclamationCircleIcon } from "@heroicons/react/24/outline";
...
const BookReviews = () => {
  ...
  const handleUpdateReview = async (e) => {
    e.preventDefault();
    if (!user) {
      setMessage("Bạn cần đăng nhập để sửa đánh giá!");
      return;
    }
    ...
    try {
      ...
      setMessage("Có lỗi xảy ra khi sửa đánh giá!");
    }
  };

  const handleDelete = async (reviewId) => {
    if (!user) {
      setMessage("Bạn cần đăng nhập để xóa đánh giá!");
      return;
    }
      ...
      <div className="w-full max-w-4xl relative z-10">
        <div className="text-center mb-10 animate-slide-up">
          ...
          <p className="mt-2 text-lg sm:text-xl text-gray-600 dark:text-gray-300">
            Xem và quản lý các đánh giá sách của bạn
          </p>
        </div>
