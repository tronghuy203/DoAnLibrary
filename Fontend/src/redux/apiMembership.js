...
import {
  ...
  export const getAllMemberships = async (accessToken, dispatch, axiosJWT) => {
    ...
    try {
      ...
      dispatch(getMembershipsFailed(err.response?.data?.message || "Lỗi khi lấy danh sách gói"));
      console.error("Lỗi khi lấy danh sách gói:", err);
      return [];
    }
  };
  
  // Kiểm tra trạng thái gói thành viên của người dùng
  export const getMembershipStatus = async (accessToken, dispatch, axiosJWT) => {
    dispatch(getMembershipStatusStart());
    try {
      const res = await axiosJWT.get("http://localhost:8000/v1/membership/status", {
        headers: { token: `Bearer ${accessToken}` },
      });
      dispatch(getMembershipStatusSuccess(res.data));
      return res.data;
    } catch (err) {
      dispatch(getMembershipStatusFailed(err.response?.data?.message || "Lỗi khi kiểm tra trạng thái gói"));
  ...
  // Mua gói thành viên
  export const purchaseMembership = async (membershipId, method, accessToken, dispatch, axiosJWT) => {
    ...
    try {
      ...
      dispatch(purchaseMembershipFailed(err.response?.data?.message || "Lỗi khi mua gói"));
